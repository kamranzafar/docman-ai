...
- **Soft delete and restore**: mark a document deleted without removing anything — the Mongo
  record, revision history, and MinIO content all stay in place, and the `deleted` flag propagates
  to the indexed chunks asynchronously via the same Kafka-triggered metadata sync, after which
